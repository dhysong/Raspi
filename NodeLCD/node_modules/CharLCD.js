...
var Setup = function (callback, pin_rs, pin_e, pins_db){
	...
	for(var i=0; i < pins_db.length; i++){
		console.log(pins_db[i]);
		console.log(i == pins_db.length - 1);
		var readyCallback = i == pins_db.length - 1 ? function(){ pinReady(callback); } : function(){};
		gpioPins[i] = GPIO.export(pins_db[i], 
		{
			direction: 'out',
			ready: function(){
				readyCallback();
	...
	function pinReady(callback){
		console.log('pinReady called');
